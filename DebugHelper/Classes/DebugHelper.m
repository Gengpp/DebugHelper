        ...
        if( recognizer.state == UIGestureRecognizerStateBegan)
        {
            ...
            if(!open)
            {
                ...
                [UIView animateWithDuration:0.3 animations:^{
                    self.windowMainView.frame = CGRectMake(0, 0, self.windowMainView.frame.size.width, self.windowMainView.frame.size.height);
                }];
            }
            else
            {
                open = NO;
                [UIView animateWithDuration:0.3 animations:^{
                    self.windowMainView.frame = CGRectMake(self.windowMainView.frame.origin.x, self.windowMainView.frame.size.height, self.windowMainView.frame.size.width, self.windowMainView.frame.size.height);
    ...
    @autoreleasepool {
        ...
        if(view != _lastView)
        {
            ...
            if(recognizer.state != UIGestureRecognizerStateCancelled && recognizer.state != UIGestureRecognizerStateEnded)
            {
                ...
                UIViewController *controller = [view parentViewController];
